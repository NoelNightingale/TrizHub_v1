        ...
        function($controllerProvider, $provide, $compileProvider, $filterProvider, $injector, $animateProvider) {
            ...
            this.config = function(config) {
                ...
                if (angular.isDefined(config.modules)) {
                    if (angular.isArray(config.modules)) {
                        angular.forEach(config.modules,
            ...
            this._init = function _init(element) {
                        ...
                        function(elm) {
                            if (modulesToLoad.length === 0) {
                                ...
                                if (match) {
                                    ...
                                    angular.forEach(elm.attributes,
                                        function(attr) {
                ...
                if (modulesToLoad
                    ...
                    console
                        .error("No module found during bootstrap, unable to init ocLazyLoad. You should always use the ng-app directive or angular.boostrap when you use ocLazyLoad.");
                }
            ...
            this.$get = [
                "$log", "$rootElement", "$rootScope", "$cacheFactory", "$q",
...
// We don't know about this module, but something else might, so push it anyway.
                                    ...
                                    if (self.moduleExists(requireEntry.name)) {
                                        ...
                                        if (angular.isDefined(self.filesLoader)) {
                                            ...
                                            promisesList.push(self.filesLoader(requireEntry, localParams)
                                                .then(function() {
                                                    return self._loadDependencies(requireEntry);
                                                }));
                                        } else {
                                            return reject(new
                                                Error("Error: New dependencies need to be loaded from external files (" + requireEntry.files + "), but no loader has been defined."));
                                        }
                                        return;
                                    } else if (angular.isArray(requireEntry)) {
                                        var files = [];
                                        angular.forEach(requireEntry,
                                            function(entry) {
                        ...
                        inject: function inject(moduleName) {
                            ...
                            if (modulesToLoad.length > 0) {
                                ...
                                var loadNext = function loadNext(moduleName) {
                                    ...
                                    self._loadDependencies(moduleName, localParams)
                        ...
                        getRequires: function getRequires(module) {
                            var requires = [];
                            angular.forEach(module.requires,
    ...
    angular.module("oc.lazyLoad")
        .config([
            "$provide", function($provide) {
                ...
                [
                    "$delegate", "$q", "$window", "$interval", function($delegate, $q, $window, $interval) {
                        var uaCssChecked = false,
                            useCssLoadPatch = false,
                            anchor = $window.document.getElementsByTagName("head")[0] ||
                        ...
                        $delegate.buildElement = function buildElement(type, path, params) {
                            ...
                            default:
                                ...
                                deferred.reject(new Error('Requested type "' +
                                    type +
                            ...
                            if (params.insertBefore) {
                                var element = angular.element(angular.isDefined(window.jQuery)
                                    ? params.insertBefore
                            ...
                            if (type == "css") {
                                if (!uaCssChecked) {
                                    ...
                                    if (/iP(hone|od|ad)/.test($window.navigator.platform)) {
                                        ...
                                        var iOSVersion =
                                            parseFloat([parseInt(v[1], 10), parseInt(v[2], 10), parseInt(v[3] || 0, 10)]
                                                .join("."));
                    ...
                    "$delegate", "$q", function($delegate, $q) {
                        ...
                        $delegate.filesLoader = function filesLoader(config) {
                            ...
                            var pushFile = function pushFile(path) {
                                ...
                                if (angular.isUndefined(cachePromise) || params.cache === false) {
                                    ...
                                    if ((m = /^(css|less|html|htm|js)?(?=!)/.exec(path)) !== null) {
                                        ...
                                        file_type = m[1];
                                        path = path.substr(m[1]
                                            .length +
                                            1,
                                            path.length); // Strip the type from the path
                                    }
                                    ...
                                    if (!file_type) {
                                        if ((m = /[.](css|less|html|htm|js)?((\?|#).*)?$/.exec(path)) !== null) {
                                            ...
                                            file_type = m[1];
                                        } else if (!$delegate.jsLoader.hasOwnProperty("ocLazyLoadLoader") &&
                            ...
                            if (templatesFiles.length > 0) {
                                ...
                                $delegate.templatesLoader(templatesFiles,
                                    function(err) {
                        ...
                        $delegate.load = function(originalModule) {
                            ...
                            $delegate.filesLoader(config, localParams)
                                .then(function() {
                                        $delegate.inject(null, localParams)
                                            .then(function(res) {
                                                    deferred.resolve(res);
                                                },
                                                function(err) {
    ...
    angular.module("oc.lazyLoad")
        .config([
            "$provide", function($provide) {
                $provide.decorator("$ocLazyLoad",
                [
                    "$delegate", "$templateCache", "$q", "$http", function($delegate, $templateCache, $q, $http) {
                        /**
                                    ...
                                    $http.get(url, params)
                                        .success(function(data) {
                                            if (angular.isString(data) && data.length > 0) {
                                                angular.forEach(angular.element(data),
                                                    function(node) {
                                                        if (
                                        ...
                                        .error(function(err) {
                                            deferred
                                                .reject(new
                                                    Error('Unable to load template file "' + url + '": ' + err));
                                        });
                                });
                            return $q.all(promises)
                                .then(function() {
