...
angular.module("AngularApp")
    .directive("ngDropdownMultiselect",
    [
        ...
        function($filter, $document, $compile, $parse) {
            ...
            return {
                ...
                template: function(element, attrs) {
                    const checkboxes = attrs.checkboxes ? true : false;
                    const groups = attrs.groupBy ? true : false;
                    var template = "<div class=\"multiselect-parent btn-group dropdown-multiselect fullWidth\">";
                    template +=
                    ...
                    template +=
                        "<li ng-hide=\"!settings.showCheckAll || settings.selectionLimit > 0\"><a data-ng-click=\"selectAll()\"><span class=\"glyphicon glyphicon-ok\"></span>  {{texts.checkAll}}</a>";
                    template +=
                        "<li ng-show=\"settings.showUncheckAll\"><a data-ng-click=\"deselectAll();\"><span class=\"glyphicon glyphicon-remove\"></span>   {{texts.uncheckAll}}</a></li>";
                    template +=
                    ...
                    if (checkboxes) {
                        ...
                        template +=
                            "<span data-ng-class=\"{'glyphicon glyphicon-ok': isChecked(getPropertyForObject(option,settings.idProp))}\"></span> {{getPropertyForObject(option, settings.displayProp)}}</a>";
                    }
                ...
                link: function($scope, $element, $attrs) {
                    ...
                    $scope.getButtonText = function() {
                        ...
                        ($scope.selectedModel && $scope.selectedModel.length > 0 ||
                        (angular.isObject($scope.selectedModel) && _.keys($scope.selectedModel).length > 0))) {
                            if ($scope.settings.smartButtonMaxItems > 0) {
                                ...
                                angular.forEach($scope.options,
                                    function(optionItem) {
                                        if ($scope.isChecked($scope
                                            .getPropertyForObject(optionItem, $scope.settings.idProp))) {
                                            const displayText = $scope
                                                .getPropertyForObject(optionItem, $scope.settings.displayProp);
                                            const converterResponse = $scope.settings
                                                .smartButtonTextConverter(displayText, optionItem);
                                ...
                                let totalSelected;
                                if ($scope.singleSelection) {
                                    totalSelected = ($scope.selectedModel !== null &&
                                            angular.isDefined($scope.selectedModel[$scope.settings.idProp]))
                                        ? 1
                                        : 0;
                                } else {
                                    totalSelected = angular.isDefined($scope.selectedModel)
