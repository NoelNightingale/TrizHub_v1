USE [TrizHub_Prod]
GO
/****** Object:  StoredProcedure [dbo].[lsp_getTimesheetReportDetail]    Script Date: 2026/08/26 09:28:51 ******/
SET ANSI_NULLS ON
GO
...
set @sql = 'select ts.UserAccountId, u.FirstName + '' '' + u.Surname as Contact, ts.DateEntry, c.EntityName Client, p.ProjectNumber, p.ProjectName, pt.Name, p.Billable,
		...
		t.TeamName, a.ActivityName, ts.Comments, ts.Hours,
		COALESCE(brProject.Rate, brClient.Rate, brDefault.Rate) as Rate
...
left join BillingRates brProject
  on brProject.UserAccountId = ts.UserAccountId
 and brProject.ProjectId = ts.ProjectId
 and brProject.ClientId is null
 and ts.DateEntry >= brProject.StartDate
 and ts.DateEntry <= brProject.EndDate
left join BillingRates brClient
  on brClient.UserAccountId = ts.UserAccountId
 and brClient.ClientId = p.ClientId
 and brClient.ProjectId is null
 and ts.DateEntry >= brClient.StartDate
 and ts.DateEntry <= brClient.EndDate
left join BillingRates brDefault
  on brDefault.UserAccountId = ts.UserAccountId
 and brDefault.ClientId is null
 and brDefault.ProjectId is null
 and ts.DateEntry >= brDefault.StartDate
 and ts.DateEntry <= brDefault.EndDate'
...
-- exec lsp_getTimesheetReportDetail '01 Jun 2024', '30 Jun 2024', 'All', 'All', 'All', true
